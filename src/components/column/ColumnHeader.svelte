...
<div class="column-header shrink-0 flex flex-col" style="--accent: {config.color};">
  ...
  >
    ...
    {#if config.channel === 'hybridTimeline'}
      <CircleDotDashed class="w-3.5 h-3.5 shrink-0 opacity-70" style="color: {config.color};" aria-hidden="true" />
    {:else if config.channel === 'globalTimeline'}
      <Globe class="w-3.5 h-3.5 shrink-0 opacity-70" style="color: {config.color};" aria-hidden="true" />
    {:else}
      <svg
