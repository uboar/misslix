<script lang="ts">
  ...
  import { Home, Lock, AtSign, WifiOff, Hash } from 'lucide-svelte';
...
<div class="note-body text-xs leading-relaxed text-base-content/90">
  ...
  {#if channel || visibilityIcon || isLocalOnly}
    <div class="flex items-center gap-1.5 mb-0.5 flex-wrap min-h-0">
      ...
      {#if isLocalOnly}
        <span
          class="local-only-badge inline-flex items-center gap-0.5 text-[0.6rem] text-info/60 font-medium shrink-0"
          title="ローカルのみ (連合なし)"
