<script lang="ts">
  ...
  import {
    ...
    FileText, Clock, AlertCircle, Shield, MessageCircle, Trophy, Download, LogIn, Key, AppWindow,
  } from 'lucide-svelte';
  ...
  type Props = {
    ...
    /** マージ通知タイムライン用: アカウントカラー (表示する場合) */
    sourceColor?: string;
    /** マージ通知タイムライン用: アカウントラベル */
    sourceLabel?: string;
  };

  let { notification, runtime, sourceColor, sourceLabel }: Props = $props();
...
>
  ...
  <div class="flex-1 min-w-0">
    <!-- マージ通知: アカウントラベル -->
    {#if sourceLabel}
      <p class="text-[0.6rem] text-base-content/30 mb-0.5 truncate">{sourceLabel}</p>
    {/if}
